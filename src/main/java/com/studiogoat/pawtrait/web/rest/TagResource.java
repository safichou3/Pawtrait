...
import com.studiogoat.pawtrait.service.TagService;
...
/**
 * REST controller for managing {@link com.studiogoat.pawtrait.domain.Tag}.
 */
@RestController
@RequestMapping("/api")
public class TagResource {
     ...
     */
    @PutMapping("/tags/{id}")
    public ResponseEntity<TagDTO> updateTag(@PathVariable(value = "id", required = false) final String id, @RequestBody TagDTO tagDTO)
        ...
        return ResponseEntity
            .ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, tagDTO.getId()))
            .body(result);
    }
    ...
    public ResponseEntity<TagDTO> partialUpdateTag(
        ...
        return ResponseUtil.wrapOrNotFound(result, HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, tagDTO.getId()));
    }
