...
import com.studiogoat.pawtrait.service.LikeService;
...
/**
 * REST controller for managing {@link com.studiogoat.pawtrait.domain.Like}.
 */
@RestController
@RequestMapping("/api")
public class LikeResource {
     ...
     */
    @PutMapping("/likes/{id}")
    public ResponseEntity<LikeDTO> updateLike(@PathVariable(value = "id", required = false) final String id, @RequestBody LikeDTO likeDTO)
        ...
        return ResponseEntity
            .ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, likeDTO.getId()))
            .body(result);
    }
    ...
    public ResponseEntity<LikeDTO> partialUpdateLike(
        ...
        return ResponseUtil.wrapOrNotFound(
            result,
            HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, likeDTO.getId())
        );
    }
     ...
     */
    @GetMapping("/likes/{id}")
    public ResponseEntity<LikeDTO> getLike(@PathVariable String id) {
