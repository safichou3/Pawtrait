...
export default class PhotoTag extends mixins(JhiDataUtils) {
  ...
  public photoTags: IPhotoTag[] = [];

  public isFetching = false;

  public mounted(): void {
    this.retrieveAllPhotoTags();
  }
  ...
  public retrieveAllPhotoTags(): void {
        ...
        res => {
          if (res.data && res.data.length > 0) {
            for (let i = 0; i < res.data.length; i++) {
              this.photoTags.push(res.data[i]);
            }
  ...
  public prepareRemove(instance: IPhotoTag): void {
  ...
  public removePhotoTag(): void {
    this.photoTagService()
      .delete(this.removeId)
      .then(() => {
        const message = 'A PhotoTag is deleted with identifier ' + this.removeId;
